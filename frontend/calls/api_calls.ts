...
//     const tempChallenges: string[] = data?.result?.data?.map(
//       (item: { data: { objectId: string } }) => item.data.objectId
//     );
...
// export const get_object_from_id = async (id: string, object_name: string) => {
//   try {
//     const response = await fetch(
//       `https://api.shinami.com/node/v1/${process.env.NEXT_PUBLIC_ACCESS_TOKEN}`,
//       {
